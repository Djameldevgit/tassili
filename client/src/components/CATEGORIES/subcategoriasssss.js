...
const Subcategories = ({ postData, handleChangeInput }) => {
  ...
  const vehiculesCategories = [
    { id: 'automobiles', name: t('vehicules.categories.automobiles'), icon: <span style={{ fontSize: '24px' }}>🚗</span>, color: 'primary' },
  ...
  const handleSelectChange = (e) => {
    ...
    console.log('🔄 Subcategories - Cambio detectado:', {
      mainCategory,
      activeTab,
      value
    });
    ...
    if (mainCategory === 'immobilier') {
      if (activeTab === 'operation') {
        // Guardar en articleType
        handleChangeInput({
          target: { name: 'articleType', value }
        });
      } else if (activeTab === 'property') {
        // Guardar en subCategory
        handleChangeInput({
          target: { name: 'subCategory', value }
        });
      }
  ...
  // 🎯 SOLO UNA FUNCIÓN DE RENDER PARA IMMOBILIER
  const renderImmobilierContent = () => {
    const currentTabData = activeTab === 'operation' 
      ? immobilierOperations 
      : immobilierProperties;
    
    // Determinar valor actual para el select
    const currentValue = activeTab === 'operation'
      ? (postData.articleType || '')
      : (postData.subCategory || '');
    
    const selectLabel = activeTab === 'operation'
      ...
      <div className="mt-2">
        ...
        <div className="mb-3">
          <div className={`nav nav-tabs ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <button
              ...
              type="button"
              disabled={!postData.articleType}
              style={{ 
                opacity: !postData.articleType ? 0.5 : 1,
                cursor: !postData.articleType ? 'not-allowed' : 'pointer'
              }}
            >
              🏠 {t('type_property', 'Type de bien')}
              {!postData.articleType && ' (sélectionnez d\'abord une opération)'}
            </button>
          </div>
        </div>
        ...
        <Form.Group className="mb-3">
          ...
          <Form.Select
            ...
            dir={isRTL ? 'rtl' : 'ltr'}
            disabled={activeTab === 'property' && !postData.articleType}
          >
  ...
  // 🎯 Renderizar SOLO Immobilier con la nueva función
  if (mainCategory === 'immobilier') {
    return (
...
// 🎯 Para OTRAS CATEGORÍAS (todas excepto immobilier)
