...
export const FieldConfig = {
    ...
    'immobilier': {
      // Configuración por OPERATION (articleType)
      'vente': {
        baseFields: ['superficie', 'nombrePieces', 'prix'],
        conditional: {
          'appartement': ['etage', 'ascenseur', 'parking', 'meuble'],
          'villa': ['jardin', 'piscine', 'garage', 'etages'],
          'terrain': ['superficie', 'zonage', 'viabilise', 'pente'],
          'local': ['superficie', 'activiteAutorisee', 'vitrine'],
          'immeuble': ['nombreEtages', 'nombreAppartements'],
          'bungalow': ['superficie', 'mobilite'],
          'terrain_agricole': ['superficie', 'zonage', 'viabilise'],
          // Para otras propiedades, usar configuración por defecto
          'default': ['superficie', 'description']
        }
      },
      'location': {
        baseFields: ['superficie', 'loyer', 'caution', 'dureeBail'],
        conditional: {
          'appartement': ['etage', 'chargesComprises', 'meuble'],
          'villa': ['jardin', 'piscine', 'etages'],
          'default': ['superficie']
        }
      },
      // ... otros articleTypes
  ...
  export const getVisibleFields = (mainCategory, subCategory, articleType) => {
    ...
    console.log('🔍 Buscando campos para:', { mainCategory, subCategory, articleType });
  
    // CASO ESPECIAL: Para immobilier, articleType es la "subcategoría principal"
    if (mainCategory === 'immobilier' && articleType) {
      const config = FieldConfig[mainCategory]?.[articleType];
  
      if (!config) {
        console.warn('⚠️ No config encontrada para immobilier:', articleType);
        return [];
      }
      ...
      // Si hay subCategory (type de bien), buscar campos condicionales
      if (subCategory && config.conditional && config.conditional[subCategory]) {
        conditionalFields = config.conditional[subCategory];
      } else if (config.conditional && config.conditional['default']) {
        conditionalFields = config.conditional['default'];
      }
  
      const allFields = [...new Set([...baseFields, ...conditionalFields])];
      console.log('🔍 Campos encontrados para immobilier:', allFields);
      return allFields;
    }
    ...
    return [...new Set([...baseFields, ...conditionalFields])];
  };